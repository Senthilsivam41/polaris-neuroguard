# Cross-Agent Coordination — Claude Code (`claude-code`)

You (`agent_id = claude-code`) have been added to this project's AutoClaw
...
- State:   .autoclaw/orchestrator/comms/inboxes/claude-code/_state/
- Done:    .autoclaw/orchestrator/comms/inboxes/claude-code/processed/

## Lifecycle (per cycle)
1. Write a heartbeat to
   .autoclaw/orchestrator/comms/heartbeats/claude-code.json
...
## How you are revived
- loop_mechanism:    slash-loop
- keepalive_template: templates/keepalive/claude-code.md

When your heartbeat goes stale, the orchestrator's `/orchestrate revive
claude-code` renders that template (with your last task + stall duration)
and delivers it via the slash-loop path.
